...
def download_csv_files():
    ...
    for object_name, url in {
        ...
        try:
            ...
            logger.info(f"Found {object_name}' in the {bucket_name}.")
        except S3Error as err:
            if err.code == "NoSuchKey":
                logger.info(f"Downloading {object_name}...")
                object_content = download_csv(
                    url,
                    object_name,
                    bucket_name
                )
                ...
                # This handles other errors (like no permissions or bucket not found)
...
def download_zip_files():
    ...
    if first_item:
        logger.info(f"Data already imported in the {bucket_name}.")
    else:
